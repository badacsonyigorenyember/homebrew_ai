...
|---|---|---|---|
| ⭐ **4.5** *(scheduled, unchanged)* | WF4 + `tool-search-brewing-knowledge` + `mem.chat_turns`, de-enumerated prompt (§7.1) | `tier1_routing.py`: knowledge **30/30**, total ≥ **73/84** · Tier C backlog for books 0a–4 · the hard refusal check · record the latency baseline | delete both workflows |
| **4.6 — foundations** | `obs` schema · `wf-step-llm` + `wf-step-compute` engines · prompts migrated into `obs.prompts` · profiles table | a WF4 turn produces `obs.runs`/`obs.steps` joinable to `mem.chat_turns` · ⭐ **`tier1` re-run is unchanged** — nothing user-facing moved, so the score must not move | `drop schema obs cascade` |
| ⭐ **4.7 — `brainstorm.pairing`** | capability #2 first: ⭐ **it needs no math and no reference data**, so it proves the engine against the corpus you already have | 12-case eval: ⛔ **0 ungrounded candidates survive step 4** · every cited claim resolves to a real chunk · ⭐ `tier1` re-run with a second tool — **this is the two-tool selection number Phase 2 could never measure** | unregister the tool |
