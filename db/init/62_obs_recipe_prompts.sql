...
-- ---------------------------------------------------------------------------
-- ⭐ propose v11: Stage B's routing reaches the model.
--
-- brew.f_resolve_request has been answering "lactose -> catalogue id 109" in SQL
-- since 35_nlq_evidence.sql landed, and v10 used that answer for exactly one
-- thing: detecting an `unknown` term. The model was still left to find the id
-- for itself in a 302-row list.
--
-- ⛔ `measured` 2026-09-16 on eval case R07 -- "a 20 litre sweet stout at 6% ABV
-- with lactose for body, plus vanilla, poppy seeds and a dark rum" -- the recipe
-- shipped with NO LACTOSE. Not in "items", not in "uncatalogued_additions", and
-- not in "not_available" either, so nothing on the sheet told the brewer it was
-- missing. Vanilla went into the v8 slot in the same run, though vanilla had
-- just been given catalogue id 691. Both ids were resolved before the call and
-- neither was ever shown.
--
-- This is D41's lesson one more time: a fact the pipeline can COMPUTE is not a
-- thing to ask the model to look up. The routing block states the verdict and
-- the id for every term the brewer named, and says which output field it belongs
-- in -- catalogued to "items", known_uncatalogued and thin to
-- "uncatalogued_additions", unknown to "not_available".
--
-- ⚠️ Backed by code either way, because a prompt rule is not a guarantee: F5 in
-- `Validate proposal` reports any catalogued term the brewer named that is
-- absent from the finished recipe. Reported, not forced in -- inserting it needs
-- a quantity, and the only honest source for one is the cohort.
-- ---------------------------------------------------------------------------
INSERT INTO obs.prompts (name, version, body, active, notes) VALUES
('formulate.recipe/propose', 11, $body$
...
⭐ WHAT THE BREWER NAMED, AND WHERE EACH ONE GOES. This was resolved against the
catalogue and against 174,000 corpus recipes BEFORE you were called. It is not a
suggestion and you do not need to search the list below for these -- the id is
right here. ⛔ EVERY ONE OF THESE MUST APPEAR IN YOUR ANSWER, in the place this
block sends it. Leaving one out entirely is the single worst thing you can do:
the brewer asked for it by name.
{{routing}}
...
- conflicts: array of plain sentences, each naming one way the request cannot be
  satisfied as written. Empty array if the request is coherent.
- rationale: one sentence, at most 30 words, on the shape of the grist.
$body$, false, 'v11: adds {{routing}} -- Stage B''s per-term verdict and resolved catalogue id, stated before the catalogue rather than left for the model to find. Measured on R07: v10 shipped a sweet stout with no lactose at all despite lactose being resolved to id 109 before the call. Backed by F5, which reports any catalogued term the brewer named that is missing from the finished recipe.')
ON CONFLICT (name, version) DO NOTHING;
