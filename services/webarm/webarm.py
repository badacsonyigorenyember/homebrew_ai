...
def web_arm(query, top_k=3, candidates=4):
    t0=time.time()
    ...
    if not chunks: return {"query":query,"rows":[],"skipped":skipped,"dropped":dropped,"secs":round(time.time()-t0,1)}
    qv = embed([query])[0]
    ...
    return {"query":query,"skipped":skipped,"dropped":dropped,"secs":round(time.time()-t0,1),
            "rows":[{"score":round(c["score"],3),"url":c["url"],
                     "heading":" > ".join(c["headings"][:2]),
                     "text":" ".join(c["raw"].split())[:200]} for c in chunks[:top_k]]}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            return self._send(200, web_arm(query, top_k=top_k, candidates=cands))
...
if __name__ == "__main__":
    if len(sys.argv) > 1:                      # CLI mode, for probing
        for q in sys.argv[1:]:
            r = web_arm(q)
            print(f"\n### {q}   ({r['secs']}s, {len(r['skipped'])} skipped, {r.get('dropped',0)} short-dropped)")
